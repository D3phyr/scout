<template>
      <div :class="$style.overlay" @click="!$props.main && $emit('closePopup')">
        <div :class="$style.popupContainer" @click.stop>
          <v-icon v-if="!$props.main" :class="[$style.closeButton, themeWhite && $style.invert]" :name="'closePopup'" @click="$emit('closePopup')"></v-icon>
          <div v-if="$props.main" :class="$style.title">Отсутствует интернет-соединение, обновите страницу</div>
          <div v-if="!$props.main" :class="$style.title">У оператора разорвана связь с матчем</div>
          <v-btn v-if="!$props.main" @click="$emit('closePopup')" :color="'black'">
              ОК
          </v-btn>
        </div>
      </div>
...
export default {
    name: 'v-popupDisconnectOperator',
    props: {
        array: {
            type: Array,
            default: () => [],
